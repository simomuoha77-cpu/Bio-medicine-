...
const uploadFields = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.fieldname === "thumbnail") {
      ...
      else cb(new Error("Thumbnail must be an image (jpg/png/webp)"), false);
    } else {
      if (ALLOWED_TYPES[file.mimetype]) cb(null, true);
      else cb(new Error("Only PDF, Word (.docx), PowerPoint (.pptx), Excel (.xlsx) files allowed"), false);
    }
  },
...
// Deduct point and save chat history (browser calls Gemini directly)
...
app.post("/api/xadmin/pdfs",
  ...
  async (req, res) => {
    try {
      ...
      await logActivity("upload", `Uploaded: ${title} (${detectedType})`, "admin001");
